...
python scripts/import_oews_to_mongo.py
...
mongosh "$MONGODB_URL/$TEMP_DATABASE" --eval "
    var counts = {
...
if [ $? -ne 0 ]; then
    ...
    mongosh "$MONGODB_URL/$TEMP_DATABASE" --eval "db.dropDatabase()"
    exit 1
fi
...
DB_EXISTS=$(mongosh "$MONGODB_URL" --quiet --eval "db.getMongo().getDBNames().includes('$MONGODB_DATABASE')")
...
if [ "$DB_EXISTS" = "true" ]; then
    ...
    mongosh "$MONGODB_URL/$MONGODB_DATABASE" --eval "db.dropDatabase()"
fi
...
mongosh "$MONGODB_URL/$TEMP_DATABASE" --eval "db.dropDatabase()" 2>/dev/null || true

# Ask about deleting backup
echo ""
echo "Backup database created: $BACKUP_DATABASE"
echo "To delete backup later, run:"
echo "  mongosh $MONGODB_URL/$BACKUP_DATABASE --eval 'db.dropDatabase()'"
echo ""
