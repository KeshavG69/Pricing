...
class ContactFormRequest(BaseModel):
    ...
    company: str = Field(
        default="", max_length=100, description="Company name (optional)"
    )
...
async def send_contact_form(request: ContactFormRequest):
    ...
    This endpoint:
    1. Sends email to support@priceiq.org with Reply-To set to user's email
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Failed to send message. Please try again or email us directly at support@priceiq.org",
        )
